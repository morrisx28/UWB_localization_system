...
namespace CppLinuxSerial {
	...
	int SerialPort::Open() {
		...
		if (fileDesc_ == -1) {
			...
			std::cout << "Could not open device " << device_ << ". Is the device name correct and do you have read/write permission?" << std::endl;
		    return -1;
		}
	...
	void SerialPort::Write(char *data, int size) {
        if(state_ != State::OPEN)
